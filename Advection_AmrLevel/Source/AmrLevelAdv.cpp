...
AmrLevelAdv::variableSetUp ()
{
    ...
    */
    ...
    for (int i = 0; i<NUM_STATE; ++i){
		desc_lst.setComponent(Phi_Type, i, "phi", bc, 
...
//	  const FArrayBox& statein = S_new[mfi];
	...
	//	Display_vector(&level, &cur_time, AMREX_ARLIM_3D(lo), AMREX_ARLIM_3D(hi),
...
//	   std::cout<<"I am working here"<<std::endl;
    ...
    const Real* prob_lo = geom.ProbLo();
	  ...
	  amrex::Print() << "dx = "<< dx[0]<<"dy = "<<dx[1]<<std::endl;
	  amrex::Print() << "cur_time = "<< cur_time <<std::endl;
      amrex::Print()<<"Prob_lo = "<< prob_lo[0]<< prob_lo[1]<< prob_lo[2]<<std::endl;
...
//	   std::cout<<"I am working here"<<std::endl;
	 //
	 //Adapt Ricardo & Alexis Approach
	 //
	 
	 int DIRPARAM =0;
//	 std::cout<<"I amworking here"<<std::endl;
	
	 FluxANDUpdate(&DIRPARAM, &time, bx.loVect(), bx.hiVect(),
                  BL_TO_FORTRAN_3D(statein), 
	 ...
	 //   Display_vector(&level, &time, bx.loVect(), bx.hiVect(),  BL_TO_FORTRAN_3D(stateout) );
	    
	}
    }
    

    // State with ghost cells Copy the results from xsweep into Sborder
    MultiFab Sborder2(grids, dmap, NUM_STATE, NUM_GROW);
    FillPatch(*this, Sborder2, NUM_GROW, cur_time, Phi_Type, 0, NUM_STATE);
...
#pragma omp parallel
	...
	for (MFIter mfi(S_new, true); mfi.isValid(); ++mfi)
	{
	    ...
	    const FArrayBox& statein = Sborder2[mfi];
	    FArrayBox& stateout      =   S_new[mfi];
	    ...
	    int DIRPARAM =1;
	//  std::cout<<"I am about to enter y direcction sweep"<<std::endl;
	 	 
  //    amrex::Print() << "cur_time = "<< cur_time <<std::endl;
	//  	  amrex::Print() << "ctr_time = "<< ctr_time <<std::endl;
	//     	  amrex::Print() << "time = "<< time <<std::endl; 
		 FluxANDUpdate(&DIRPARAM, &time, bx.loVect(), bx.hiVect(),
                  BL_TO_FORTRAN_3D(statein), 
				  BL_TO_FORTRAN_3D(stateout),
    ...
    //
	//Write a plotfile at the end of iteration
	//
	...
	for (MFIter mfi(S_new, true); mfi.isValid(); ++mfi)
	{
		...
		//
		// THE GHOST REGIONS MUST BE TAKEN INTO ACCOUNT FOR MAX VELOCITY
		//
	//	amrex::Print() << " Plot file at level " << level << std::endl;
		Plot_File(&level, &cur_time, BL_TO_FORTRAN_3D(statein),
					dx);
...
AmrLevelAdv::estTimeStep (Real)
{
    ...
    MultiFab Sborder(grids, dmap, NUM_STATE, NUM_GROW);
    FillPatch(*this, Sborder, NUM_GROW, cur_time, Phi_Type, 0, NUM_STATE);
...
#pragma omp parallel reduction(min:dt_est)
	...
	for (MFIter mfi(S_new, true); mfi.isValid(); ++mfi)
	{
		...
		// THE GHOST REGIONS MUST BE TAKEN INTO ACCOUNT FOR MAX VELOCITY
		//
		dt_function_2d(&level, &cur_time, 
					BL_TO_FORTRAN_3D(statein),
					dx, &dt_est_temp);
		
	//	std::cout<<"dt_est_temp"<<dt_est_temp<<std::endl;		
		
		if(dt_est>dt_est_temp){
			dt_est = dt_est_temp;
		}
                       ...
                       int          ngrow)
{
    ...
    const Real cur_time = state[Phi_Type].curTime();
    	...
    	// State with ghost cells
   MultiFab Sborder(grids, dmap, S_new.nComp(), S_new.nGrow()+1);
   FillPatch(*this, Sborder, S_new.nGrow()+1, cur_time, Phi_Type, 0, NUM_STATE);
	...
	Real Max_grad = 0;	
{
	...
	//Calculate the maxmium Density Gradient 
	//The function uses central difference scheme therefore ghost cells are required 
	...
	for (MFIter mfi(S_new,true); mfi.isValid(); ++mfi)
	{
	    ...
	    //Call Fortran Function to Calculate max gradient 
	    
	    Max_Gradient( BL_TO_FORTRAN_3D(Sborder[mfi]),
						AMREX_ARLIM_3D(tlo),
						 AMREX_ARLIM_3D(thi),
                      &level, AMREX_ZFILL(dx), &MAXG);
                      
        Max_grad = std::max(Max_grad, MAXG);
         
	    
    }
    }

  //  ParallelDescriptor :: ReduceRealMax(Max_grad);

	
	//
	//Here we calculate local gradiant and compare with Max_gradint 
	//Also set tags
	//
...
#pragma omp parallel
	...
	for (MFIter mfi(S_new,true); mfi.isValid(); ++mfi)
	{
		...
		state_error(tptr,  AMREX_ARLIM_3D(tlo), AMREX_ARLIM_3D(thi),
			...
			&tagval, &clearval, 
			AMREX_ARLIM_3D(tilebx.loVect()),
			AMREX_ARLIM_3D(tilebx.hiVect()), 
			AMREX_ZFILL(dx), 
			AMREX_ZFILL(prob_lo), &time, &level, &Max_grad);
