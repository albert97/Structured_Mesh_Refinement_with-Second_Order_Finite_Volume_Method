...
module compute_flux_module
  ...
  private :: HLLC_FLUX, MUSCL_TVD_FLUX, &
...
!	write(*,*)"ph(32,0)",phi(32,0,  1)
  ...
  subroutine HLLC_FLUX( DIRPARAM, QL, QR, FLUX )
	...
	double precision, intent(in)::QRv
  ...
  PSTAR = MAX( 0.0, PPVRS )
   ...
   QKSTAR (1) = ft
   QKSTAR (4) = ft * ( EK / RHOK + ( SStar - VNormR )*(  &
                 & SStar + PK/( RHOK*( SR - VNormR ) ) ) ) 
 ...
 subroutine DATA_RECONSTRUCTION( QL, QC, QR, QRECON ) 
	...
	do i = 1, nVarQ
		...
		SlopeLR= 2.d0*BetaCR/( 1.d0 - Omega + &
		          & ( 1.d0 + Omega )*r  
		call SLOPE_LIMITER(r, SlopLR, SlopeLimiter)
		
	    di = 0.5d0*( 1.d0 + Omega )*di1+ &
		      & 0.5d0*( 1.d0 - Omega )*di2		
		sdi = SLOPE_LIMITER * di
...
end subroutine DATA_RECONSTRUCTIO
  !----------------------   SLOPE LIMITER   ----------------------------
