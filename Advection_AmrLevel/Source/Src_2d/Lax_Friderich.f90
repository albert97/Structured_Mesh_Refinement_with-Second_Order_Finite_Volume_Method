...
subroutine advect(time, lo, hi, &
  ...
  !dummy array to store valyues after x sweep
  double precision :: u_xout(ui_lo(1):ui_hi(1),ui_lo(2):ui_hi(2))
...
PRINT *, "uo_loX= ",uo_lo(1), "uo_loY= ", uo_lo(2), "uo_hix= ",uo_hi(1), "uo_hi= ", uo_hi(2)
...
PRINT *, "vx(i-1,j) = ", vx(lo(1)-1,j), "uin(i-1, j)", uin(lo(1)-1, j), " vx(i+1,j)",  vx(lo(1)+1,j), " uin(i+1, j) ",  uin(lo(1)+1, j)
	...
	!Copy values from uin into u_xout
	do j = ui_lo(2), ui_hi(2)
		do i = ui_lo(1), ui_hi(1)
	!	PRINT *, "u_xout(i, j)",u_xout(i, j), "=", "uin(i, j)", uin(i, j)
			u_xout(i, j) = uin(i, j)
		end do
	end do
	
 ! Use Lax_Firdrich Scheme for updates 
  do  j = lo(2), hi(2)
		do i = lo(1) , hi(1)
		
	!	PRINT *, "vx(i-1,j) = ", vx(i-1,j), "uin(i-1, j)", uin(i-1, j), " vx(i+1,j)",  vx(i+1,j), " uin(i+1, j) ",  uin(i+1, j)
		
			u_xout(i,j) = 0.5d0  * ( 1.0d0 + (dtdx(1)) * vx(i-1,j)) *  uin(i-1, j) + 0.5d0  * (  1.0d0 - (dtdx(1)) * vx(i+1,j)) *  uin(i+1, j)
         
			
			!0.5d0  * ( 1 + (dtdx) * vx(i-1,j)) *  uin(i-1, j) + 0.5d0  * ( 1 + (dtdx) * vx(i+1,j)) *  uin(i+1, j)
 ...
 do  j = lo(2), hi(2)
	 do i = lo(1) , hi(1)
		...
		!PRINT *, "vy(i,j-1) = ", vy(i,j-1), "u_xout(i, j-1)", u_xout(i, j-1), " vx(i,j+1)",  vy(i,j+1), " u_xout(i, j+1) ",  u_xout(i, j+1)

		uout(i,j) = 0.5d0  * ( 1.0d0 + (dtdx(2)) * vy(i,j-1)) *  u_xout(i, j-1) + 0.5d0  * (  1.0d0 - (dtdx(2)) * vy(i,j+1)) *  u_xout(i, j+1)
