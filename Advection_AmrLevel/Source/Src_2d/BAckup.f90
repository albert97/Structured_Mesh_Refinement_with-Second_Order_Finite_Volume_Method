...
!		PRINT *, "vx(i-1,j) = ", vx(i-1,j), "uin(i-1, j)", uin(i-1, j), " vx(i+1,j)",  vx(i+1,j), " uin(i+1, j) ",  uin(i+1, j)
			...
			uout(i,j) = 0.5d0  * ( 1 + (dtdx(1)) * vx(i-1,j)) *  uin(i-1, j) + 0.5d0  * ( 1 + (dtdx(1)) * vx(i+1,j)) *  uin(i+1, j)
 ...
 do  j = lo(2), hi(2)
	 do i = lo(1) , hi(1)
		...
		PRINT *, "vy(i,j-1) = ", vy(i,j-1), "uin(i, j-1)", uin(i, j-1), " vx(i,j+1)",  vy(i,j+1), " uin(i, j+1) ",  uin(i, j+1)

		uout(i,j) = 0.5d0  * ( 1 + (dtdx(2)) * vy(i,j-1)) *  uin(i, j-1) + 0.5d0  * ( 1 + (dtdx(2)) * vy(i,j+1)) *  uin(i, j+1)
